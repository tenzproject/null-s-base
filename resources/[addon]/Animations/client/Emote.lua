...
local lang = Config.Animations.MenuLanguage
...
for i = 1, #emoteTypes do
    ...
    for emoteName, emoteData in pairs(RP[emoteType]) do
        ...
        if Config.Animations.AdultEmotesDisabled and emoteData.AdultAnimation then shouldRemove = true end
    ...
    CreateThread(function()
        ...
        while AnimationThreadStatus and (IsInAnimation or PtfxPrompt) do
            ...
            if IsInAnimation then
                ...
                if not Config.Animations.AllowPunching then
...
if Config.Animations.EnableCancelKeybind then
    RegisterKeyMapping("emotecancel", Translate('register_cancel_emote'), "keyboard", Config.Animations.CancelEmoteKey)
end
...
CreateThread(function()
    ...
    TriggerEvent('chat:addSuggestion', '/emote', Translate('play_emote'),
        { { name = "emotename", help = Translate('help_command') },
            { name = "texturevariation", help = Translate('help_variation')  } })
    if Config.Animations.SqlKeybinding then
...
-- if Config.Animations.MenuKeybindEnabled then
--     RegisterCommand('emoteui', function() OpenEmoteMenu() end, false)
--     RegisterKeyMapping("emoteui", "Open animations menu", "keyboard", Config.Animations.MenuKeybind)
-- else
...
if Config.Animations.HandsupEnabled then
    RegisterCommand('handsup', function()
        if IsPedInAnyVehicle(PlayerPedId(), false) and not Config.Animations.HandsupKeybindInCarEnabled and not InHandsup then
            return
        end
    ...
    function Handsup()
        ...
        if InHandsup then
            ...
            CurrentExportEmote = nil
            ClearPedSecondaryTask(PlayerPedId())
            if Config.Animations.PersistentEmoteAfterHandsup and IsInAnimation then
    ...
    if Config.Animations.HandsupKeybindEnabled then
        RegisterKeyMapping("handsup", Translate('handsup'), "keyboard", Config.Animations.HandsupKeybind)
    end
...
function EmoteCancel(force)
    ...
    TriggerServerEvent("setAnimServer", nil)
end
...
function EmoteCommandStart(source, args, raw)
    if #args > 0 then
        ...
        if (IsPedSwimming(PlayerPedId()) or IsPedSwimmingUnderWater(PlayerPedId())) and not Config.Animations.AllowInWater then
            TriggerEvent('chat:addMessage', {
                color = {255, 0, 0},
                multiline = true,
        ...
        elseif RP.AnimalEmotes[name] ~= nil then
            if Config.Animations.AnimalEmotesEnabled then
                CheckAnimalAndOnEmotePlay(RP.AnimalEmotes[name], name)
                return
            else
...
function OnEmotePlay(EmoteName, name, textureVariation)
    ...
    if not Config.Animations.AllowedInCars and InVehicle == 1 then
        return
    end
    ...
    if Config.Animations.AdultEmotesDisabled and EmoteName.AdultAnimation then
    ...
    if Config.Animations.CancelPreviousEmote and IsInAnimation and not ExitAndPlay and not EmoteCancelPlaying then
        ExitAndPlay = true
    ...
    if Config.Animations.DisarmPlayer then
    ...
    if ChosenDict == "MaleScenario" or ChosenDict == "Scenario" or ChosenDict == "ScenarioObject" then
        ...
        if ChosenDict == "MaleScenario" then if InVehicle then return end
            ...
            TriggerServerEvent("setAnimServer", name)
            return
...
function OnEmotePlayPed(EmoteName, name, textureVariation)
    if not Config.Animations.PreviewPed then return end
    ...
    -- if Config.Animations.CancelPreviousEmote and IsInAnimation and not ExitAndPlay and not EmoteCancelPlaying then
    if Config.Animations.CancelPreviousEmote and not ExitAndPlay and not EmoteCancelPlaying then
        ExitAndPlay = true
        DebugPrint("Canceling previous emote and playing next emote")
...
--    return CurrentExportEmote -- Ou toute autre variable contenant les données d'animation
--end)
