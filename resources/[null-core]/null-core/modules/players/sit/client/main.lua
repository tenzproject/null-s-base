...
exports("canStopChair", function(bool)
    canStop = bool
end)


function SitOnAChair(force, msg) -- Vive le nom de la fonction mdr
    if force then
        canStop = false
    end
    if isSit and canStop then
        isSit = false
    ...
    elseif not isSit then
        ...
        if closetDst < 2.0 then
            ...
            --TaskPlayAnim(PlayerState.ped, "anim@heists@prison_heistunfinished_biztarget_idle", "target_idle", 1.0, 1.0, -1, 2, 0, false, false, false)
            ...
            if force then
                ...
                SetCamCoord(cam, GetOffsetFromEntityInWorldCoords(closetObj, 0.0, -0.6, 1.8))
                local rotation = GetEntityRotation(closetObj)
                print(json.encode(rotation))
                if rotation.z >= 170.0 then 
                    rotation = vector3(rotation.x, rotation.y, 0.0) 
                end
                SetCamRot(cam, -70.0, 0.0, -(rotation.z), 0)
