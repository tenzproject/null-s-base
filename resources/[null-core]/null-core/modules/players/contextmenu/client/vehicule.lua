    ...
    Action_Config = {
        Vehicule = {
            ...
            },
            {
                ...
                CloseOnClick = true,
                Blocked = function()
                    -- Vérifier que c'est bien un BMX
                    ...
                    return modelName ~= "bmx"
                end,
                OnClick = function() 
                    if LastEntityHit and DoesEntityExist(LastEntityHit) then
                        local model = GetEntityModel(LastEntityHit)
                        local modelName = string.lower(GetDisplayNameFromVehicleModel(model))
                        if modelName == "bmx" then
                            -- Supprimer le véhicule
                            DeleteEntity(LastEntityHit)
                            -- Donner l'item au joueur
                            TriggerServerEvent('null:tutorial:pickupBmx')
                            ESX.ShowNotification('~g~BMX ramassé !~s~~n~Il a été ajouté à votre inventaire.')
                        end
                    end
                end,
            },
            ...
            
            {
                ...
                Action = {
                  ...
                  --  {
                   --     permissions = "CUSTOM_VEHICLE",
                  --      'Changer la plaque',
                   --     KeepNuiFocus = true,
                   --     function()
                     --       local plaque = null.fct.input('Plaque du véhicule (MAX 8 Caractères)')
                    --        if plaque then
                 ...
                 --   },
                    {
                        ...
                        "Full perf", 
                        function() 
                            TriggerServerEvent("veh_actioncontext", "custom", NetworkGetNetworkIdFromEntity(LastEntityHit))
                        end
                    ...
                    },
                    {
                        ...
                        "Booster le véhicule", 
                        function() 
                            SetEntityMaxSpeed(LastEntityHit, 10000.0/3.6) 
                            SetVehicleCheatPowerIncrease(LastEntityHit, 1.8)
                            SetVehicleEnginePowerMultiplier(LastEntityHit, 2.0)
                            SetVehicleHandlingFloat(LastEntityHit, "CHandlingData", "fInitialDriveForce", 1.8)
                        end
