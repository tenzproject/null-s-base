function SimpleNotify(message)
    if Config.Animations.NotificationsAsChatMessage then
...
function DebugPrint(...)
    if Config.Animations.DebugDisplay then
        print(...)
    end
end
...
ShowPedMenu = function(zoom)
    ...
    if not ShowPed then
        CreateThread(function()
            ...
            if not zoom then
                while ShowPed do
                    local world, normal = GetWorldCoordFromScreenCoord(0.32135417461395, 0.57)
                    local depth = 5
                    ...
                    for _, position in ipairs(positionBuffer) do averagedTarget = averagedTarget + position end
