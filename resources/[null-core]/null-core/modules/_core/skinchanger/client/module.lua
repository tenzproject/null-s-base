...
function GetPlayerSkinData(dataName)
...
function LoadDefaultModel(malePed, cb)
	...
	Citizen.CreateThread(function()
		...
		if IsModelInCdimage(characterModel) and IsModelValid(characterModel) then
			...
			-- IMPORTANT: Récupérer le nouveau ped APRÈS SetPlayerModel
...
function ApplySkin(skin, clothes)
	...
	-- Vérifier que le drawable existe avant d'appliquer (évite l'erreur "null drawable")
	local function safeSetVariation(ped, componentId, drawableId, textureId)
		if drawableId and drawableId >= 0 and GetNumberOfPedDrawableVariations(ped, componentId) > drawableId then
			SetPedComponentVariation(ped, componentId, drawableId, textureId or 0, 0)
		end
	end
	...
	pcall(function()
		...
		SetPedEyeColor				(playerPed,			math.min(math.max(tonumber(Character['eye_color']) or 0, 0), 29))			-- Eyes color
		...
		safeSetVariation(playerPed, 8, Character['tshirt_1'], Character['tshirt_2'])		-- Tshirt
		safeSetVariation(playerPed, 11, Character['torso_1'], Character['torso_2'])		-- torso parts
		safeSetVariation(playerPed, 3, Character['arms'], Character['arms_2'])			-- Arms
		safeSetVariation(playerPed, 10, Character['decals_1'], Character['decals_2'])	-- decals
		safeSetVariation(playerPed, 4, Character['pants_1'], Character['pants_2'])		-- pants
		safeSetVariation(playerPed, 6, Character['shoes_1'], Character['shoes_2'])		-- shoes
		safeSetVariation(playerPed, 1, Character['mask_1'], Character['mask_2'])		-- mask
		safeSetVariation(playerPed, 9, Character['bproof_1'], Character['bproof_2'])	-- bulletproof
		safeSetVariation(playerPed, 7, Character['chain_1'], Character['chain_2'])		-- chain
		safeSetVariation(playerPed, 5, Character['bags_1'], Character['bags_2'])		-- Bag
	end)

	-- Fonction sécurisée pour les props
	local function safeSetProp(ped, propId, drawableId, textureId)
		if drawableId == nil or drawableId == -1 then
			ClearPedProp(ped, propId)
		elseif GetNumberOfPedPropDrawableVariations(ped, propId) > drawableId then
	...
	safeSetProp(playerPed, 0, Character['helmet_1'], Character['helmet_2'])		-- Helmet
	safeSetProp(playerPed, 1, Character['glasses_1'], Character['glasses_2'])	-- Glasses
	safeSetProp(playerPed, 6, Character['watches_1'], Character['watches_2'])	-- Watches
	safeSetProp(playerPed, 7, Character['bracelets_1'], Character['bracelets_2'])	-- Bracelets
end

RegisterNetEvent('Null:skinchanger:loadDefaultModel', function(loadMale, cb)
	LoadDefaultModel(loadMale, cb)
end)

RegisterNetEvent('Null:skinchanger:getData', function(cb)
...
RegisterNetEvent('Null:skinchanger:loadDefaultSkin', function()
...
RegisterNetEvent('Null:skinchanger:change', function(key, val)
	...
	if key == 'sex' then
		TriggerEvent('Null:skinchanger:loadSkin', Character)
	else
		ApplySkin(Character)
		-- null.DebugPrint("Skin changed: " .. key .. " = " .. val)
	end
	if key == "bags_1" then
		TriggerEvent("Null:changeBag", val)
	end
end)

RegisterNetEvent('Null:skinchanger:getSkin', function(cb)
	cb(Character)
end)

RegisterNetEvent('Null:skinchanger:modelLoaded', function()
...
RegisterNetEvent('Null:skinchanger:loadSkin', function(skin, cb)
	if skin['sex'] ~= LastSex then
		LoadSkin = skin

		if skin['sex'] == 0 then
			TriggerEvent('Null:skinchanger:loadDefaultModel', true, cb)
		else
			TriggerEvent('Null:skinchanger:loadDefaultModel', false, cb)
		end
	else
		ApplySkin(skin)
...
RegisterNetEvent('Null:skinchanger:loadClothes:NoSkin', function(clothesSkin)
	if Character['sex'] ~= LastSex then
		...
		if Character['sex'] == 0 then
			TriggerEvent('Null:skinchanger:loadDefaultModel', true)
		else
			TriggerEvent('Null:skinchanger:loadDefaultModel', false)
		end
	else
		ApplySkin(Character, clothesSkin)
	end

	LastSex = Character['sex']
end)

RegisterNetEvent('Null:skinchanger:loadClothes', function(playerSkin, clothesSkin)
...
local stopLoadall = false

local toLoad = {
    hair_1			= GetNumberOfPedDrawableVariations		(playerPed, 2) - 1,
    hair_2			= GetNumberOfPedTextureVariations		(playerPed, 2, 0) - 1,
    hair_color_1	= GetNumHairColors()-1,
    hair_color_2	= GetNumHairColors()-1,
    ears_1			= GetNumberOfPedPropDrawableVariations	(playerPed, 2) - 1,
    ears_2			= GetNumberOfPedPropTextureVariations	(playerPed, 2, -1 - 1),
    tshirt_1		= GetNumberOfPedDrawableVariations		(playerPed, 8) - 1,
    tshirt_2		= GetNumberOfPedTextureVariations		(playerPed, 8, 0) - 1,
    torso_1			= GetNumberOfPedDrawableVariations		(playerPed, 11) - 1,
    torso_2			= GetNumberOfPedTextureVariations		(playerPed, 11, 0) - 1,
    decals_1		= GetNumberOfPedDrawableVariations		(playerPed, 10) - 1,
    decals_2		= GetNumberOfPedTextureVariations		(playerPed, 10, 0) - 1,
    arms			= GetNumberOfPedDrawableVariations		(playerPed, 3) - 1,
    arms_2			= 10,
    pants_1			= GetNumberOfPedDrawableVariations		(playerPed, 4) - 1,
    pants_2			= GetNumberOfPedTextureVariations		(playerPed, 4, 0) - 1,
    shoes_1			= GetNumberOfPedDrawableVariations		(playerPed, 6) - 1,
    shoes_2			= GetNumberOfPedTextureVariations		(playerPed, 6, 0) - 1,
    mask_1			= GetNumberOfPedDrawableVariations		(playerPed, 1) - 1,
    mask_2			= GetNumberOfPedTextureVariations		(playerPed, 1, 0) - 1,
    bproof_1		= GetNumberOfPedDrawableVariations		(playerPed, 9) - 1,
    bproof_2		= GetNumberOfPedTextureVariations		(playerPed, 9, 0) - 1,
    chain_1			= GetNumberOfPedDrawableVariations		(playerPed, 7) - 1,
    chain_2			= GetNumberOfPedTextureVariations		(playerPed, 7, 0) - 1,
    bags_1			= GetNumberOfPedDrawableVariations		(playerPed, 5) - 1,
    bags_2			= GetNumberOfPedTextureVariations		(playerPed, 5, 0) - 1,
    helmet_1		= GetNumberOfPedPropDrawableVariations	(playerPed, 0) - 1,
    helmet_2		= GetNumberOfPedPropTextureVariations	(playerPed, 0, -1) - 1,
    glasses_1		= GetNumberOfPedPropDrawableVariations	(playerPed, 1) - 1,
    glasses_2		= GetNumberOfPedPropTextureVariations	(playerPed, 1, 0 - 1),
    watches_1		= GetNumberOfPedPropDrawableVariations	(playerPed, 6) - 1,
    watches_2		= GetNumberOfPedPropTextureVariations	(playerPed, 6, -1) - 1,
    bracelets_1		= GetNumberOfPedPropDrawableVariations	(playerPed, 7) - 1,
    bracelets_2		= GetNumberOfPedPropTextureVariations	(playerPed, 7, -1 - 1)
}

RegisterNetEvent("null:skinchanger:loadall", function()
	stopLoadall = false
	local data = GetMaxVals()
	for k,v in pairs(data) do
		if toLoad[k] == nil then goto continue end
		if stopLoadall == true then break end
		index = 1
		if v ~= -1 and v ~= 0 and (tonumber(v) - index) ~= 0 then
			while (tonumber(v) - index) > 0 do
				if stopLoadall == true then break end
				TriggerEvent("Null:skinchanger:change", k, index)
				index = index + 1
				print(index, v, (tonumber(v) - index))
				Wait(100)
			end
		else
			TriggerEvent("Null:skinchanger:change", k, 0)
		end
		Wait(100)
		::continue::
	end
	TriggerServerEvent("null:streams:preload:finish")
end)

RegisterNetEvent("null:skinchanger:stoploadall", function()
	stopLoadall = true
end)
