				...
				if result[1] ~= nil then
					MySQL.Async.fetchAll('SELECT * FROM account_info WHERE license = @license', {
						...
						if result2[1] ~= nil then
							local targetplayername = result2[1].name
							if source ~= 0 then
								sourceplayername = GetPlayerName(source)
							else
								sourceplayername = "Console"
							end
							license = result2[1].license
							identifier = result2[1].identifier or nil
							liveid = result2[1].live
							xblid  = result2[1].xbl
							discord = result2[1].discord
							playerip = result2[1].ip

							null.logs.send("Logs","Le joueur "..sourceplayername.."\nA ban le joueur "..targetplayername, "ban")
							if duree > 0 then
								ban(source,license,identifier,liveid,xblid,discord,playerip,targetplayername,sourceplayername,duree,reason,0)
							else
...
function cmdunban(source, args)
	if args[1] then
	local target = tonumber(args[1])
	MySQL.Async.fetchAll('SELECT * FROM banlist WHERE banid = @banid', 
	{
		['@banid'] = target
	}, function(data)
		if data[1] then
			if #data > 1 then
				...
				MySQL.Async.execute(
				'DELETE FROM banlist WHERE banid = @banid',
				{
				  ['@banid']  = args[1]
				},
					function ()
					...
					null.logs.send("Logs","Le joueur "..sourceplayername.."\nA unban le joueur "..data[1].targetplayername, "unban")
					TriggerEvent('bansql:sendMessage', source, data[1].targetplayername .. Text.isunban)
				end)
			end
		else
			TriggerEvent('bansql:sendMessage', source, Text.invalidbanid)
		end
	...
	if target ~= "" then
		MySQL.Async.fetchAll('SELECT * FROM banlist WHERE banid = @banid', 
		{
			...
			if data[1] then
				if #data < 50 then
					for i=1, #data, 1 do
						TriggerEvent('bansql:sendMessage', source, data[i].banid.." "..data[i].targetplayername)
					end
				else
					TriggerEvent('bansql:sendMessage', source, Text.toomanyresult)
				end
...
exports('banoffline', function(args)
	cmdbanoffline(0, args)
end)
...
function ban(source,license,identifier,liveid,xblid,discord,playerip,targetplayername,sourceplayername,duree,reason,permanent)
	print("^7[^5VolShield^7] "..targetplayername.." a était banni pour "..reason.."")
	local xPlayer = ESX.GetPlayerFromId(source)
	null.logs.send("Ban","Le joueur : "..targetplayername.."\na était banni pour : "..reason.."\n pendant : "..duree.."","ban")
	...
	MySQL.Async.fetchAll('SELECT * FROM banlist WHERE targetplayername like @playername', 
	{
		...
		if not data[1] then
			...
			--sendToDiscord(Config.Logs["ban"], message)
...
function deletebanned(license) 
	null.logs.send("Unban","La license : "..license.." a était débanni","unban")
	MySQL.Async.execute(
