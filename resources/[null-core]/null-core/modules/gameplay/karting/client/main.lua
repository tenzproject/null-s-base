    ...
    while true do
        ...
        if dist < 150 then
            if IsPedSittingInVehicle(PlayerPedId(), KartingVehicle) then
                ...
                ESX.ShowNotification('Vous avez quitté la zone du karting, Votre véhicule à été saisie')
            end
        end
        if dist2 < 50 then
            if IsPedSittingInVehicle(PlayerPedId(), KartingVehicle) then
...
function OpenMenuPlayerKarting()
    ...
    while menu do
        ...
        if not ActivityKarting then
            RageUI.Separator('~s~Activité - Karting')
            RageUI.Button('Commencer l\'activité', nil, {RightLabel = '~g~500$'}, true, {
                onSelected = function()
                    ActivityKarting = true
                    ESX.Game.SpawnVehicle("veto2", vector3(-153.08323669434,-2147.4096679688,16.705001831055), nil, function(vehicle)
            ...
            RageUI.Separator('Activité - Karting')
