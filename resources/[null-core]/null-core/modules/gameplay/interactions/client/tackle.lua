...
RegisterNetEvent('esx:tackle:getTackled')
AddEventHandler('esx:tackle:getTackled', function(target)
...
RegisterNetEvent('esx:tackle:playTackle')
AddEventHandler('esx:tackle:playTackle', function()
	local playerPed = PlayerPedId()
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlPressed(0, 21) and IsControlPressed(0, 246) and not isTackling and GetGameTimer() - lastTackleTime > 10 * 1000 and not PlayerIsDead then
			Citizen.Wait(10)
			...
			if distance ~= -1 and distance <= 3 and not isTackling and not isGettingTackled and not IsPedInAnyVehicle(PlayerPedId()) and not IsPedInAnyVehicle(GetPlayerPed(closestPlayer)) then
				...
				TriggerServerEvent('esx:tackle:tryTackle', GetPlayerServerId(closestPlayer))
			end
		end
	end
end)