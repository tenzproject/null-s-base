...
if #Config.Computer.UsablePositions > 0 then
    CreateThread(function()
        while true do
            if UIOpen then
                Wait(500)
            ...
            for i = 1, #Config.Computer.UsablePositions, 1 do
                local currentDistance = #(playerCoords - Config.Computer.UsablePositions[i])
            ...
            if nearestDistance and nearestDistance < 2.0 then
                CustomDrawMarker(Config.Computer.UsablePositions[nearestIndex])
                CustomHelpNotification(GetLocale("start_computer"))

                if IsControlJustPressed(0, 51) then
                    OpenUI(Config.Computer.UsablePositions[nearestIndex])
                end
                Wait(0)
            else
                Wait(500)
...
if Config.Computer.DataHeists.Enabled then
    if Config.Computer.DataHeists.DisplayArea then
        CreateThread(function()
            for coords, radius in pairs(Config.Computer.DataHeists.Areas) do
    ...
    function GetDataHeistAtCoords(coords)
        local nearest
        local nearestDistance
        for heistCoords, radius in pairs(Config.Computer.DataHeists.Areas) do
    ...
    function DataHeistAreaCheck(heistCoords)
        CreateThread(function()
            local ped = PlayerPedId()
            local radius = Config.Computer.DataHeists.Areas[heistCoords]
...
local propsLength = #Config.Computer.UseProps
if propsLength > 0 then
    CreateThread(function()
        local propsList = Config.Computer.UseProps

        if Config.Computer.TargetSystem then
            if Config.Computer.TargetType == "ox" then
