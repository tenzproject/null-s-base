...
// Fonction pour initialiser le bureau
function initializeDesktop(visibleApps) {
    console.log('Initializing desktop with visible apps:', visibleApps);
    ...
    // Nettoyer les applications existantes
    apps = [];
    openedApps = [];
    ...
    Object.entries(Applications).forEach(entry => {
        ...
        // Vérifier si l'application doit être visible
        const isVisible = visibleApps ? visibleApps[appName] !== false : true;
        
        // Ajouter l'icône si l'application doit être visible
        if (isVisible) {
            try {
                ...
                console.error(`Error creating icon for ${appName}:`, error);
            }
        }

        // Toujours ajouter le code HTML de l'application si elle est utilisable
        if (appData.usable) {
            try {
                apps.push(appName);
                desktop.innerHTML += appData.appCode;
            } catch (error) {
                console.error(`Error adding app code for ${appName}:`, error);
            }
        }
    });

    // Configurer les événements pour chaque application
    apps.forEach(app => {
        try {
            // Ajouter l'événement onclick sur l'icône si elle existe
            const appIcon = document.getElementById(app);
            if (appIcon) {
                appIcon.onclick = () => OpenApp(app);
            }

            // Configurer les boutons de l'application
            const quitBtn = document.getElementById(app+"-quit");
            const minimizeBtn = document.getElementById(app+"-minimize");
            if (quitBtn) quitBtn.onclick = () => CloseApp(app);
            if (minimizeBtn) minimizeBtn.onclick = () => MinimizeApp(app);

            // Configurer le conteneur de l'application
            ...
            console.error(`Error setting up events for ${app}:`, error);
        }
    });

    // Déclencher un événement personnalisé pour notifier que le bureau est initialisé
    const event = new CustomEvent('desktopInitialized');
...
window.addEventListener("message", (event) => {
    if (event.data.type === "show") {
        ...
        // Appliquer le thème laboratory si nécessaire
        const desktop = document.getElementById("desktop");
        if (event.data.applications && event.data.applications["laboratory-upgrades"]) {
            //document.documentElement.setAttribute("data-theme", "laboratory");
            desktop.classList.add("laboratory");
            editTheme({
                "--main-color": "#0D0D0D",
                "--lighter-color": "#151515",
                "--darker-color": "#0A0A0A",
                "--darkest-color": "#050505",
                "--app-minimize-color": "#80BDCF",
                "--app-exit-color": "#E75656",
            })
        } else {
            //document.documentElement.removeAttribute("data-theme");
            desktop.classList.remove("laboratory");
        ...
        Load(true, GetLocale("os_session"), 100, () => {
            Load(true, GetLocale("os_boot"), 150, () => {
                // Initialiser le bureau avec les applications visibles
                initializeDesktop(event.data.applications);
                ...
                fetch(`https://${GetParentResourceName()}/getApplicationsData`,
                {
                    ...
                    for (const [appName, appData] of Object.entries(data)) {
                        if (Applications[appName] && Applications[appName].usable && !Applications[appName].hide) {
                            if (appName == "market") {
                    ...
                    if (data.wallpapers) {
                        ...
                        if (wallpapersContainer) {
                            ...
                            const currentWallpaper = localStorage.getItem("wallpaper") || "Dark Dune";
                            
                            data.wallpapers.forEach(wallpaper => {
                                let wallpaperElement = document.createElement("div");
                                wallpaperElement.className = "wallpaper-item";
                                if (currentWallpaper === wallpaper.image) {
                                    wallpaperElement.classList.add("active");
                                }
                                
                                const previewStyle = wallpaper.image === "none" 
                                    ? "background: rgba(255,255,255,0.1);" 
                                    : `background-image: url('${wallpaper.image}'); background-size: cover; background-position: center;`;
                                ...
                                wallpaperElement.onclick = () => setWallpaper(wallpaper.image);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    unusableApps.forEach(app => {
        ...
        if (element) {
            element.onclick = () => MessageBox("error", GetLocale("os_error"), GetLocale("os_fake_error").replace("{1}", app));
        }
    });
...
const editTheme = (themeData) => {
    ...
    let newTheme = document.getElementById("theme-"+themeData["--main-color"].toLowerCase());
    if (newTheme) {
        newTheme.innerHTML = "•";
    }
...
/**
 * Set wallpaper
 * @param {string} wallpaperImage wallpaper image path
 */
const setWallpaper = (wallpaperImage) => {
    const desktop = document.getElementById("desktop");
    if (!desktop) return;
    ...
    document.querySelectorAll(".wallpaper-item").forEach(item => {
        ...
        if (preview) {
            ...
            if ((wallpaperImage === "none" && !bgImage) || 
                (bgImage && bgImage.includes(wallpaperImage))) {
    ...
    // Apply wallpaper
    if (wallpaperImage === "none") {
        desktop.style.background = "";
        desktop.style.backgroundImage = "";
    } else {
        desktop.style.backgroundImage = `url('${wallpaperImage}')`;
        desktop.style.backgroundSize = "cover";
        desktop.style.backgroundPosition = "center";
        desktop.style.backgroundRepeat = "no-repeat";
    }
    ...
    localStorage.setItem("wallpaper", wallpaperImage);
}
