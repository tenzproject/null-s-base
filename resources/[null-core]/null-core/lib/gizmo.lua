...
local dataview = require 'lib.dataview'
...
local function textUILoop()
    CreateThread(function()
        while gizmoEnabled do
            Wait(100)
            local scaleText = (enableScale and '[S]     - Mode d\'échelle  \n') or ''
            lib.showTextUI(
                ('Mode actuel : %s | %s  \n'):format(currentMode, (isRelative and 'Relatif') or 'Monde') ..
                '[R]     - Changer de mode  \n' ..
                scaleText ..
                '[Q]     - Relatif/Monde  \n' ..
                '[LALT]  - Ancrer au sol  \n' ..
                '[ENTER] - Fin de la modification  \n'
            )
        end
        lib.hideTextUI()
    end)
...
lib.addKeybind({
    ...
    description = 'Switch betwen Translate and Rotate mode',
    defaultKey = 'R',
    onPressed = function(self)
        if not gizmoEnabled then return end
...
Citizen.CreateThread(function()
    --[[while true do
        DrawGlowSphere(-75.027397, -819.216125, 326.175171, 
            10.0, 
            255, 
            0, 
            0, 
