...
const MAP_IMAGE_URL = 'nui://null-cache/images/satellite.jpg';
        ...
        {!loading && (
          ...
          >
            ...
            <div
              className="idev-tmap-stage"
              style={{
                // 1. On sort du flux pour ignorer les sécurités CSS
                position: 'absolute',
                top: '50%',
                left: '50%',

                // 2. La taille native multipliée (la garantie anti-flou)
                width: stageSize.w ? `${stageSize.w * zoom}px` : '100%',
                height: stageSize.h ? `${stageSize.h * zoom}px` : '100%',

                // 3. On centre parfaitement (-50%, -50%) PUIS on applique ton panning
            ...
            >
              <img
                ...
                style={{
                  ...
                  width: '100%',
                  height: '100%',
                  // 'fill' garantit qu'il n'y a aucun pixel vide, car notre stage a déjà le ratio parfait
                  objectFit: 'fill'
                }}
                onLoad={(e) => {
                  const t = e.target as HTMLImageElement;
              ...
              >
                {/* --- Reste du code pour les polygones --- */}
                {territories.map(t => {
                  const poly = (t.territoryPoints && t.territoryPoints.length >= 3) ? t.territoryPoints : t.points;
                  if (!poly || poly.length < 3) return null;
                  const pts = poly.map(p => {
