Citizen.CreateThread(function()
    if Config.ArmWrestling.showBlipOnMap then
        for _, info in pairs(Config.ArmWrestling.props) do
            info.blip = AddBlipForCoord(info.x, info.y, info.z)
            SetBlipSprite(info.blip, Config.ArmWrestling.blip.id)
            SetBlipDisplay(info.blip, 4)
            SetBlipScale(info.blip, 0.6)
            SetBlipColour(info.blip, Config.ArmWrestling.blip.colour)
            SetBlipAsShortRange(info.blip, true)
            BeginTextCommandSetBlipName("STRING")
            AddTextComponentString(Config.ArmWrestling.blip.title)
...
Citizen.CreateThread(function()
    while true do
        Wait(5000)
        for i, modelConfig in pairs(Config.ArmWrestling.props) do
            if Vdist(GetEntityCoords(PlayerPedId()), modelConfig.x,
                     modelConfig.y, modelConfig.z) < 50 then
                if place == 0 and
                    Vdist(GetEntityCoords(PlayerPedId()), modelConfig.x,
                          modelConfig.y, modelConfig.z) < 10 then
                    ESX.ShowHelpNotification(
                        ESX.Config("serverName") ..
                            "\n~g~Appuyez sur ~INPUT_CONTEXT~ pour intéragir")
                    if IsControlJustPressed(0, 38) and place == 0 then
                        local position = GetEntityCoords(PlayerPedId())
                        TriggerServerEvent('evy_arm:check_sv', position)
                    end
                end
                thisTable = GetClosestObjectOfType(modelConfig.x, modelConfig.y,
                                                   modelConfig.z, 1.5,
                                                   GetHashKey(modelConfig.model),
                                                   0, 0, 0)
                if DoesEntityExist(thisTable) then
                    SetEntityHeading(thisTable)
                    PlaceObjectOnGroundProperly(thisTable)
                else
                    thisTable = CreateObject(GetHashKey(modelConfig.model),
                                             modelConfig.x, modelConfig.y,
                                             modelConfig.z, false, false, false)
                    SetEntityHeading(thisTable)
                    PlaceObjectOnGroundProperly(thisTable)
                end
            elseif Vdist(GetEntityCoords(PlayerPedId()), modelConfig.x,
                         modelConfig.y, modelConfig.z) >= 50 then
                thisTable = GetClosestObjectOfType(modelConfig.x, modelConfig.y,
                                                   modelConfig.z, 1.5,
                                                   GetHashKey(modelConfig.model),
...
function checkFunction()
    for i, modelConfig in pairs(Config.ArmWrestling.props) do
        local table = GetClosestObjectOfType(GetEntityCoords(PlayerPedId()),
                                             1.5, GetHashKey(modelConfig.model),
                                             0, 0, 0)
...
AddEventHandler('evy_arm:start_cl', function()
    ...
    if place == 1 then
        ...
        SetEntityAnimCurrentTime(PlayerPedId(), "mini@arm_wrestling", "sweep_a",
                                 grade)
        ...
        while grade >= 0.10 and grade <= 0.90 do
            Wait(0)
            PlayFacialAnim(PlayerPedId(), "electrocuted_1",
                           "facials@gen_male@base")
            alert(text[Config.ArmWrestling.language]['tuto'] .. "~INPUT_PICKUP~")
            SetEntityAnimSpeed(PlayerPedId(), "mini@arm_wrestling", "sweep_a",
                               0.0)
            SetEntityAnimCurrentTime(PlayerPedId(), "mini@arm_wrestling",
                                     "sweep_a", grade)
            if IsControlPressed(0, 38) then
                ...
                while IsControlPressed(0, 38) do
                    Wait(0)
                    alert(text[Config.ArmWrestling.language]['tuto'] ..
                              "~INPUT_PICKUP~")
                    SetEntityAnimCurrentTime(PlayerPedId(),
        ...
        if grade >= 0.90 then
            ...
            notify("~g~" .. text[Config.ArmWrestling.language]['win'])
        elseif grade <= 0.10 then
            PlayAnim(PlayerPedId(), "mini@arm_wrestling", "win_a_ped_b", 2)
            notify("~r~" .. text[Config.ArmWrestling.language]['lose'])
        end
        Wait(4000)
    ...
    elseif place == 2 then
        ...
        SetEntityAnimCurrentTime(PlayerPedId(), "mini@arm_wrestling", "sweep_b",
                                 grade)
        PlayFacialAnim(PlayerPedId(), "electrocuted_1", "facials@gen_male@base")
        disabledControl = 1
        ...
        while grade >= 0.10 and grade <= 0.90 do
            ...
            PlayFacialAnim(PlayerPedId(), "electrocuted_1",
                           "facials@gen_male@base")
            alert(text[Config.ArmWrestling.language]['tuto'] .. "~INPUT_PICKUP~")
            SetEntityAnimSpeed(PlayerPedId(), "mini@arm_wrestling", "sweep_b",
                               0.0)
            SetEntityAnimCurrentTime(PlayerPedId(), "mini@arm_wrestling",
                                     "sweep_b", grade)
            if IsControlPressed(0, 38) then
...
AddEventHandler('evy_arm:check_cl', function(args)
    ...
    if args == 'place1' then
        ...
        for i, modelConfig in pairs(Config.ArmWrestling.props) do
            table = GetClosestObjectOfType(GetEntityCoords(PlayerPedId()), 1.5,
                                           GetHashKey(modelConfig.model), 0, 0,
                                           0)
        ...
        Wait(100)
        SetEntityCoords(PlayerPedId(), GetOffsetFromEntityInWorldCoords(table,
                                                                        -0.20,
                                                                        0.0, 0.0).x,
        ...
        while (GetEntityAnimCurrentTime(PlayerPedId(), "mini@arm_wrestling",
                                        "aw_ig_intro_alt1_a") < 0.95) do
            Wait(0)
        end
        ...
        while not started do
            ...
            alert(text[Config.ArmWrestling.language]['wait'])

            if IsControlPressed(2, 73) or IsPedRagdoll(PlayerPedId()) or
                IsControlPressed(2, 200) or IsControlPressed(2, 214) then
                SetEntityNoCollisionEntity(PlayerPedId(), table, true)
                TriggerServerEvent('evy_arm:disband_sv',
    ...
    elseif args == 'noplace' then
        ...
        notify('~r~' .. text[Config.ArmWrestling.language]['full'])
...
AddEventHandler('evy_arm:reset_cl', function()
    for i, modelConfig in pairs(Config.ArmWrestling.props) do
        local tableId = GetClosestObjectOfType(GetEntityCoords(PlayerPedId()),
                                               1.5,
                                               GetHashKey(modelConfig.model), 0,
                                               0, 0)
