...
local MecanoOpenCoords = nil
local forstaff = nil
local isInLSMarker = false
local myCar = {}
local vehicleClass = nil
local vehiclePrice = 15000

local CustomNull = {}
CustomNull.PercentNeon1 = 0
...
function resetMenu()
	if not mainMenu and PlayerData.job then
		...
		--mainMenu = RageUI.CreateMenu("", PlayerData.job.label)
		society = 'society_'..PlayerData.job.name
...
function calcCartValue()
	...
	for k, v in pairs(shopCart) do
		...
		totalCartArticle = totalCartArticle + 1
	end
...
local dev = false
function finishPurchase(job, staff)
	vehModsNew = lib.getVehicleProperties(vehPedIsIn)
	local primaryColorNew = GetVehicleCustomPrimaryColour(vehPedIsIn)
	if not dev then
		--passar fatura
		if staff then
			local totalCartValue2 = 0
			for k, v in pairs(shopCart) do
				totalCartValue2 = totalCartValue2 + v['price']
			end

			TriggerServerEvent('fpwn_customs:finishPurchase', society, vehModsNew, shopCart, playerId, shopProfit, shopReduction, autoInvoice, true)
		else
			...
			if (closestPlayer ~= -1 and closestDistance <= 3.0) or autoInvoice then
				...
				--if autoInvoice then
				--	TriggerServerEvent("Core:AddBilling", GetPlayerServerId(source), tonumber(totalCartValue2), jobname)
				--else
				--	TriggerServerEvent("Core:AddBilling", GetPlayerServerId(closestPlayer), tonumber(totalCartValue2), jobname)
				--end
				RageUI.CloseAll()
				TriggerServerEvent('fpwn_customs:finishPurchase', society, vehModsNew, shopCart, playerId, shopProfit, shopReduction, autoInvoice)
			else
				ESX.ShowNotification("~r~Pas de joueur proche")
...
function getVehicleTierMultiplier()
	for _, tier in ipairs(cfg_custom.VehicleTiers) do
...
function calcModPrice(basePrice)
	if not basePrice or type(basePrice) ~= "number" then return 0 end
	return math.round(basePrice * getVehicleTierMultiplier())
end
...
RegisterNetEvent('null:openMenuCustom-staff')
AddEventHandler('null:openMenuCustom-staff', function()
	if ESX.PlayerData.group == "user" then return end
	if not lsMenuIsShowed then
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			--if vehicleClass ~= 8 then
				vehModsOld = lib.getVehicleProperties(vehPedIsIn)
				if vehModsOld == nil then 
					ESX.ShowNotification("Votre véhicule est bug, merci de le ranger et le re sortir (1)")
					return 
				end
				...
				--CreateLSMenu(jobname, joblabel)
				forstaff = true
				OpenCustomMenu("staff", "Staff", true)
				MecanoOpenCoords = nil
...
RegisterNetEvent('null:openMenuCustom')
AddEventHandler('null:openMenuCustom', function(jobname, joblabel, jobcoords, forstaff)
	if not lsMenuIsShowed then
		local playerPed = PlayerPedId()
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			--if vehicleClass ~= 8 then
				...
				--CreateLSMenu(jobname, joblabel)
				forstaff = forstaff or false
				OpenCustomMenu(jobname, joblabel, forstaff)
				MecanoOpenCoords = jobcoords
				--print("OpenCustomMenu")
...
Citizen.CreateThread(LPH_NO_VIRTUALIZE(function()
    ...
    while true do
		if MecanoOpenCoords ~= nil and not forstaff then
			local pedcoords = GetEntityCoords(ped, false)
			local distance = Vdist(pedcoords.x, pedcoords.y, pedcoords.z, MecanoOpenCoords)

			if distance > 5 then
				ESX.ShowNotification("Vous avez quitter la zone de customisation")
				...
				ESX.Game.SetVehicleProperties(vehPedIsIn, vehModsOld)
				terminatePurchase()
...
OpenCustomMenu = function(jobname, joblabel, forstaff)
	lsMenuIsShowed = true
	society = 'society_'..jobname
	jobname = jobname
	RageUI.CloseAll()

	Wait(100)

    local mainMenu = RageUI.CreateMenu(ESX.PlayerData.job.label, "Menu ".. ESX.PlayerData.job.label)
	...
	local bodyMenu = RageUI.CreateSubMenu(mainMenu, "", "Gestion de la carosserie")
	local neonMenu = RageUI.CreateSubMenu(mainMenu, "", "Gestion des neons")
	local extrasMenu = RageUI.CreateSubMenu(mainMenu, "", "Gestion des extras")
	extrasMenu.Closed = function()
		SetVehicleDoorsShut(vehPedIsIn, false)
	end
	local colorMenu = RageUI.CreateSubMenu(mainMenu, "", "Gestion des couleurs")
	local neonMenu = RageUI.CreateSubMenu(mainMenu, "", "Gestion des neons")
	local upgradeMenu = RageUI.CreateSubMenu(mainMenu, "", "Gestion des ameliorations")
	local cartMenu = RageUI.CreateSubMenu(mainMenu, "", "Liste des achats")
    ...
    while mainMenu do
		pcall(function()
			ShowInfo(
				"Informations custom",  
				{
					{left = "Prix total", leftIcon = "fas fa-money-bill", right = totalCartValue.."$", color = "rgb(0, 255, 0)"},
					{left = "Nombre d\'article(s)", leftIcon = "fas fa-hashtag", right = totalCartArticle, color = "rgb("..ESX.Config("r")..","..ESX.Config("g")..","..ESX.Config("b")..")"},
				}
			)
		end)
        Wait(0)
		...
		RageUI.IsVisible(mainMenu, function()
			...
			RageUI.Line()
			RageUI.Button("Annuler" , nil, {
				LeftBadge = nil,
				RightBadge = nil,
				Color = { BackgroundColor = {255, 0, 0, 100} } 
			}, true, {
				onSelected = function()
			...
			RageUI.Button(_U('cart') , nil, {
				LeftBadge = nil,
				RightBadge = nil,
				Color = { BackgroundColor = {0, 255, 0, 150} } 
			}, true, {
				onSelected = function()
					--RageUI.Visible(bodyMenu, not RageUI.Visible(bodyMenu))
				end,
		...
		RageUI.IsVisible(extrasMenu, function()
				...
				RageUI.Slider("Sticker", newLivery, 10, "", false, {}, true, 
				{
					onSliderChange = function(Index)
						newLivery = Index
						SetVehicleLivery(vehPedIsIn, newLivery)
						compareMods("Sticker", "modLivery", 48, newLivery, calcModPrice(5))
					end,
				}
			)
			end, function()
		...
		RageUI.IsVisible(colorMenu, function()
				if bodyPartIndex[13]['modFrontWheels'] ~= 1 then
					local item = ESX.Table.Contains(tempColorParts['label'], _U('wheels'))
					...
					local item = ESX.Table.Contains(tempColorParts['label'], _U('wheels'))
					local itemPos = item and item or 0
					if type(itemPos) == "number" and itemPos > 1 then
				...
				RageUI.Checkbox(cfg_custom.tireSmoke['label'] .. " (" .. (calcModPrice(cfg_custom.tireSmoke['price']) .. "$" or "---") .. ")", 
					...
					{ Style = RageUI.CheckboxStyle.Tick }, 
					{
						onSelected = function(Checked)
							...
							if tyreSmokeActive then
								local item = ESX.Table.Contains(tempColorParts['label'], _U('tireSmoke'))
								local itemPos = item and item or 0
								if itemPos == 0 then
				...
				RageUI.Checkbox(cfg_custom.xenon['label'] .. " (" .. (calcModPrice(cfg_custom.xenon['price']) .. "$" or "---") .. ")", 
					...
					{ Style = RageUI.CheckboxStyle.Tick }, 
					{
						onSelected = function(Checked)
							...
							if xenonActive then
								local item = ESX.Table.Contains(tempColorParts['label'], _U('headlights'))
								local itemPos = item and item or 0
								if itemPos == 0 then
					...
					RageUI.List(_U('windows'),
						...
						true, 
						{
							onListChange = function(Index, CurrentItems)
								...
								compareMods(cfg_custom.windowTints['label1'], cfg_custom.windowTints['mod'], -1, itemIndex, calcModPrice(cfg_custom.windowTints['price']))
				...
				elseif tempColorParts['mod'][colorPartIndex] == 'tireSmoke' then 
					RageUI.Button(smokeColorIndex[1]['label'] .. " [" .. smokeColorIndex[1]['index'] .. "]", nil, {}, true, {
						onSelected = function(Index)
							local smokeRedValue = tonumber(null.fct.input('Taux de rouge : '))
					...
					RageUI.Button(smokeColorIndex[2]['label'] .. " [" .. smokeColorIndex[2]['index'] .. "]", nil, {}, true, {
						onSelected = function(Index)
							local smokeGreenValue = tonumber(null.fct.input('Taux de vert : '))
					...
					RageUI.Button(smokeColorIndex[3]['label'] .. " [" .. smokeColorIndex[3]['index'] .. "]", nil, {}, true, {
						onSelected = function(Index)
							local smokeBlueValue = tonumber(null.fct.input('Taux de bleu : '))
		...
		RageUI.IsVisible(neonMenu, function()
				...
				"Une fois séléctionner, veuillez séléctionner les couleurs R, G, B", 
					neon1, 
					{ Style = RageUI.CheckboxStyle.Tick },
					{
						onSelected = function(Checked)
				...
				if neon1 or neon2 or neon3 or neon4 then
					RageUI.Button("Couleur des néons", "Appuyer sur enter pour changer : Couleur des néons", {}, true, {
						onSelected = function(Index)
							local neoncolor = ESX.GetColorPickerInput("Couleur des néons")
							if neoncolor.r ~= nil and neoncolor.r >= 0 then
								neonIndex[1]['index'] = neoncolor.r
							...
							if neoncolor.g ~= nil and neoncolor.g >= 0 then
								neonIndex[2]['index'] = neoncolor.g
								SetVehicleNeonLightsColour(vehPedIsIn, neonIndex[1]['index'], neonIndex[2]['index'], neonIndex[3]['index'])
								if vehModsOld['neonEnabled'][1] or vehModsOld['neonEnabled'][2] or vehModsOld['neonEnabled'][3] or vehModsOld['neonEnabled'][4] then
									compareMods(cfg_custom.neons[5]['label'], cfg_custom.neons[5]['mod'], -1, Index, calcModPrice(cfg_custom.neons[5]['price']))
								end							
							end
							if neoncolor.b ~= nil and neoncolor.b >= 0 then
								neonIndex[3]['index'] = neoncolor.b
								SetVehicleNeonLightsColour(vehPedIsIn, neonIndex[1]['index'], neonIndex[2]['index'], neonIndex[3]['index'])
								if vehModsOld['neonEnabled'][1] or vehModsOld['neonEnabled'][2] or vehModsOld['neonEnabled'][3] or vehModsOld['neonEnabled'][4] then
									compareMods(cfg_custom.neons[5]['label'], cfg_custom.neons[5]['mod'], -1, Index, calcModPrice(cfg_custom.neons[5]['price']))
								end							
			...
			if menuItemCount == 0 then
				...
				if forstaff then
					...
					RageUI.Button(_U('finishPurchase') , nil, {
							...
							onSelected = function()
								if autoInvoice then
									ESX.TriggerServerCallback('Core:AddChoiceBilling', function(result)
										if result == true then
											if stop then
												return
											end
											stop = true
											finishPurchase(jobname)
										elseif result == false then
											ESX.ShowNotification('Vous avez refuser la facture !')
											return
										end
									end, nil, totalCartValue, "society_"..ESX.PlayerData.job.name)
								else
									local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
									if closestPlayer == -1 or closestDistance > 3.0 then
										...
										ESX.TriggerServerCallback('Core:AddChoiceBilling', function(result)
											if result == true then
												...
												finishPurchase(jobname)
											elseif result == false then
												ESX.ShowNotification('Le joueurs a refuser la facture')
												return
											end
										end, GetPlayerServerId(closestPlayer), totalCartValue, "society_"..ESX.PlayerData.job.name)
									end
								end
							end,
						}
        ...
        if not RageUI.Visible(mainMenu) and not RageUI.Visible(bodyMenu) and not RageUI.Visible(extrasMenu) and not RageUI.Visible(colorMenu) and not RageUI.Visible(neonMenu) and not RageUI.Visible(upgradeMenu) and not RageUI.Visible(cartMenu) then
			...
			vehPedIsIn = nil
			TriggerServerEvent('fpwn_customs:resetvh')
			if forstaff then
...
AddEventHandler('fpwn_customs:canBill', function(amount, targetId)
	--TriggerServerEvent('Null:esx_billing:sendBill', targetId, society, "Mecano", amount)
	canClose = true
	terminatePurchase()
end)
...
AddEventHandler('fpwn_customs:getVehicle', function()
	TriggerServerEvent('fpwn_customs:refreshOwnedVehicle', lib.getVehicleProperties(vehPedIsIn), "byClient")
end)