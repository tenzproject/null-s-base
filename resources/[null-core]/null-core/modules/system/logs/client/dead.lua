...
Citizen.CreateThread(LPH_NO_VIRTUALIZE(function()
	...
	while true do
		Citizen.Wait(500) -- Vérification toutes les 500ms suffit largement
		
		if IsEntityDead(PlayerState.ped) then
			local PedKiller = GetPedSourceOfDeath(PlayerState.ped)
			local killername = GetPlayerName(PedKiller)
			DeathCauseHash = GetPedCauseOfDeath(PlayerState.ped)
			...
			if DeathReason == 'committed suicide' or DeathReason == 'died' then
				killerCoords = GetEntityCoords(PedKiller, false)
		...
		while IsEntityDead(PlayerState.ped) do
			Citizen.Wait(500) -- Attendre le respawn, pas besoin de vérifier chaque frame
		end
	end
end))
