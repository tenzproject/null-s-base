...
RegisterNetEvent("null:admin:getSaveData.Players")
AddEventHandler("null:admin:getSaveData.Players", function()
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() ~= "user" then
        TriggerClientEvent("null:admin:getSaveData.Players", source, SaveData.Players.Online.List)
    end
end)
...
function WipeTable(targetId) -- Fonction qui vide la table des joueurs lors d'un wipe
    ...
    MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {
		...
		if result[1] ~= nil then
            ...
            null.DebugPrint("Final Wipe Loadout: "..json.encode(listearmeperm))
...
AddEventHandler("AdminMenu:updatestaffactif", function(bool)
    ...
    if xPlayer.getGroup() ~= "user" and SaveData.Players.Online[idunique] ~= nil then
        ...
        SaveData.Players.Online[idunique].staffmode = bool
...
function InitPlayersTable(xPlayer)
    local idunique = xPlayer.getIdunique()
    local loadout = xPlayer.getLoadout()
    for k,v in pairs(loadout) do
        local getWeap1, getWeap2 = ESX.GetWeapon(v.name)
        if getWeap2 ~= nil then 
            loadout[k].allcomponents = getWeap2.components 
        else
            loadout[k].allcomponents = {}
        end
    end
    local gamertag = false
    if SaveData.Players.Online[idunique] ~= nil and SaveData.Players.Online[idunique].gamertag ~= nil then
        gamertag = SaveData.Players.Online[idunique].gamertag
    end
    local staffmode = false
    if SaveData.Players.Online[idunique] ~= nil and SaveData.Players.Online[idunique].staffmode ~= nil then
        staffmode = SaveData.Players.Online[idunique].staffmode
    end

    local jobs = xPlayer.getJob()
    local jobs2 = xPlayer.getJob2()
    ...
    return {
        ...
        --fivem = xPlayer.fivem,
        instance = null.fct.instance.Get(xPlayer.source),
        playtime = xPlayer.getPlayTime() or 0,
        ...
        job = jobs.name,
        job2 = jobs2.name,
        afk_point = xPlayer.getAfk().point,
        vip = GetVIP(xPlayer.identifier),
        ...
        black = xPlayer.getAccount("dirtycash").money,
        inventory = xPlayer.getInventory(),
        staffmode = staffmode,
        gamertag = gamertag or false,
    }
end


CreateThread(function()
    null.fct.waitPlayerLoaded(100)
    Wait(6000)
    local xPlayers = ESX.GetPlayers();
    
    for i = 1, #xPlayers, 1 do
        local xPlayer = ESX.GetPlayerFromId(xPlayers[i]);
        SaveData.Players.Online[xPlayer.getIdunique()] = InitPlayersTable(xPlayer)
        if xPlayer.getGroup() ~= "user" then
...
AddEventHandler('esx:playerLoaded', function(source, xPlayer)
    SaveData.Players.Online[xPlayer.getIdunique()] = InitPlayersTable(xPlayer)
    SaveData.Admin.PlayersBlips[xPlayer.getIdunique()] = GetEntityCoords(GetPlayerPed(xPlayer.source))
    if xPlayer.getGroup() ~= "user" then
        stafflist[xPlayer.getIdunique()] = {
            group = xPlayer.getGroup(),
            source = xPlayer.source,
    ...
    TriggerClientEvent("null:admin:addPlayer", -1, xPlayer.getIdunique(), SaveData.Players.Online[xPlayer.getIdunique()], stafflist[xPlayer.getIdunique()])
...
AddEventHandler('playerDropped', function(reason)
    ...
    SaveData.Players.Online[xPlayer.getIdunique()] = nil
    SaveData.Admin.PlayersBlips[xPlayer.getIdunique()] = nil
    stafflist[xPlayer.getIdunique()] = nil
    TriggerClientEvent("null:admin:addPlayer", -1, xPlayer.getIdunique(), nil, stafflist[xPlayer.getIdunique()])
end)
...
AddEventHandler("null:admin:staffinit", function()
    ...
    TriggerClientEvent('null:admin:allrefresh', source, SaveData.Players.Online, reportsTable, stafflist, NumberPlayerTotalCached)
end)
...
AddEventHandler("null:admin:restart", function()
    ...
    TriggerClientEvent('null:admin:restart', -1, SaveData.Players.Online, reportsTable)
end)
...
function GetSourceByIdUnique(source)
    for k,v in pairs(SaveData.Players.Online) do
...
function refreshPlayer(target, type, Init)
    ...
    if SaveData.Players.Online[targetIdunique] == nil then return end
    if xTarget == nil then
        SaveData.Players.Online[targetIdunique] = nil
    elseif Init then
        SaveData.Players.Online[targetIdunique] = InitPlayersTable(xTarget)
    end
    if type == "forAll" then
        TriggerClientEvent("null:admin:refreshOnePlayer", -1, targetIdunique, SaveData.Players.Online[targetIdunique])
    elseif type == "forMe" then
        TriggerClientEvent("null:admin:refreshOnePlayer", source, targetIdunique, SaveData.Players.Online[targetIdunique])
    elseif type == "forAllStaff" then
        for k,v in pairs(SaveData.Players.Online) do
            if v.group == "user" then goto continue end
            TriggerClientEvent("null:admin:refreshOnePlayer", v.source, targetIdunique, SaveData.Players.Online[targetIdunique])
            ::continue::
        end
...
RegisterNetEvent("null:staff:changegamertag", function(bool, oldbool)
    ...
    if xPlayer.getGroup() == "user" then return end
    local idunique = xPlayer.getIdunique()
    if SaveData.Players.Online[idunique].gamertag == bool then return end
    SaveData.Players.Online[idunique].gamertag = bool
    ...
    for i = 1, #xPlayers, 1 do
        ...
        if xPlayer.getGroup() ~= "user" then
            stafflist[xPlayer.getIdunique()] = {
                ...
                name = xPlayer.getName(),
                nbrReport = 0,
                staffmode = SaveData.Players.Online[xPlayer.getIdunique()].staffmode,
            }
        end
    end]]

    TriggerClientEvent("null:player:refreh", source, SaveData.Players.Online)
end)

RegisterNetEvent("null:detectrppack:staff:setHave", function(pack)
    local xPlayer = ESX.GetPlayerFromId(pack)
    if SaveData.Players.Online[xPlayer.getIdunique()] == nil then return end

    SaveData.Players.Online[xPlayer.getIdunique()].havegfpack = true
    for k,v in pairs(SaveData.Players.Online) do
        local xPlayer = ESX.GetPlayerFromId(v.source)
        if xPlayer == nil then goto continue end
        if xPlayer.getGroup() == "user" then goto continue end

        TriggerClientEvent("null:admin:refreshOnePlayer", v.source, xPlayer.getIdunique(), SaveData.Players.Online[xPlayer.getIdunique()])
        ::continue::
    end
end)
...
ESX.RegisterServerCallback("null:admin:changeweaponacc", function(source, cb, idunique,weaponname, accname, type)
    ...
    if SaveData.Players.Online[idunique] == nil then cb(false) end
    local xTarget = ESX.GetPlayerFromId(SaveData.Players.Online[idunique].source)
    local theweapon = nil
    for k,v in pairs(SaveData.Players.Online[idunique].loadout) do
...
AddEventHandler("vAdminMenu:addstaffglobal", function(idunique, group, discord)
    ...
    if xTarget ~= nil then
        ...
        InitAllStaffs()
        TriggerClientEvent("null:admin:refreshOnePlayer", -1, idunique, SaveData.Players.Online[idunique], stafflist[idunique])
		...
		ESX.GetNameByIdUnique(idunique, function(result)
            ...
            InitAllStaffs()
            TriggerClientEvent("null:admin:refreshOnePlayer", -1, idunique, SaveData.Players.Online[idunique], stafflist[idunique])
            null.logs.send("Logs Staff",""..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a ajouter un nouveau staff : "..result.."(U"..idunique..") en tant que "..group,"addstaff", {idunique = xPlayer.getIdunique(), idunique_cible = idunique, name = xPlayer.getName(), name_cible = name})
            TriggerClientEvent("esx:showNotification", _src, "~b~"..result.." a bien était rajouter a la liste des staffs coter serveur !")
        end)
    end
end)
...
function ReturnPlayerId(UniqueID)
	for _,p in pairs(SaveData.Players.Online) do
...
exports('getPlayerWithUniqueID', function(idunique)
    ...
    -- if SaveData.Players.Online[idunique] == nil then return nil end

    if idunique == nil then return {id = nil} end
    if type(idunique) ~= "number" then return {id = nil} end
    if SaveData.Players.Online[idunique] == nil then return {id = nil} end
...
exports('getPlayerSourceByIdUnique', function(idunique)
    ...
    if SaveData.Players.Online[idunique] == nil then return 0 end
...
RegisterNetEvent("AdminMenu:tpPerso")
AddEventHandler("AdminMenu:tpPerso", function(target, coords, label)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    local xTarget = ESX.GetPlayerFromId(target)
    if xPlayer.getGroup() == "user" then
...
RegisterServerEvent("null:staff:setStreamer", function(idunique, bool)
    local xPlayer = ESX.GetPlayerFromId(source)
    local xTarget = ESX.GetPlayerFromId(ReturnPlayerId(idunique).source)
    if Config.GroupeHighPerm[xPlayer.getGroup()] ~= true then return end
    if xTarget == nil then
        if bool then
            MySQL.Async.execute("UPDATE `users` SET `streamer` = '1' WHERE `idunique` = @idunique", {['@idunique'] = idunique}, function()  end)
            if SaveData.Players.Online.List[idunique] ~= nil then
                SaveData.streamerList[idunique] = {name = SaveData.Players.Online.List[idunique].name, idunique = SaveData.Players.Online.List[idunique].idunique}
            end
        else
        ...
        if bool then
            ...
            SaveData.Players.Online[xTarget.getIdunique()].streamer = true
        else
            SaveData.connectedStreamerList[xTarget.getIdunique()] = nil
            SaveData.Players.Online[xTarget.getIdunique()].streamer = false
        end
    end
end)
...
RegisterNetEvent("AdminMenu:removeCash")
AddEventHandler("AdminMenu:removeCash", function(target, amount)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    local xTarget = ESX.GetPlayerFromId(target)
    ...
    xTarget.removeAccountMoney("cash", tonumber(amount))
...
RegisterNetEvent("AdminMenu:removeBank")
AddEventHandler("AdminMenu:removeBank", function(target, amount)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    local xTarget = ESX.GetPlayerFromId(target)
    ...
    xTarget.removeAccountMoney("bank", tonumber(amount), {title = 'Retrait Admin', description = 'Retrait par un administrateur', category = 'other'})
    TriggerClientEvent("esx:showNotification", _src, "Vous avez retirer ~b~"..amount.."$~s~ à ~b~"..xTarget.getName())
    TriggerClientEvent("esx:showNotification", target, "Un admin vous a retirer ~b~"..amount.."$")
    TriggerClientEvent("AdminMenu:serverInteractFinish", _src)
    null.logs.send("Logs Staff",""..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a retirer "..amount.."$ (bank) au joueur : "..xTarget.getName().." (U"..xTarget.getIdunique()..")", "remove-account", {idunique = xPlayer.getIdunique(), idunique_cible = xTarget.getIdunique(), name = xPlayer.getName(), name_cible = xTarget.getName()})
end)

RegisterNetEvent("AdminMenu:removeBlack")
AddEventHandler("AdminMenu:removeBlack", function(target, amount)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    local xTarget = ESX.GetPlayerFromId(target)
    ...
    xTarget.removeAccountMoney("dirtycash", tonumber(amount))
    TriggerClientEvent("esx:showNotification", _src, "Vous avez retirer ~r~"..amount.."$~s~ à ~b~"..xTarget.getName())
    TriggerClientEvent("esx:showNotification", target, "Un admin vous a retirer ~r~"..amount.."$")
    TriggerClientEvent("AdminMenu:serverInteractFinish", _src)
    null.logs.send("Logs Staff",""..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a retirer "..amount.."$ (dirtycash) au joueur : "..xTarget.getName().." (U"..xTarget.getIdunique()..")", "remove-account", {idunique = xPlayer.getIdunique(), idunique_cible = xTarget.getIdunique(), name = xPlayer.getName(), name_cible = xTarget.getName()})
end)
...
AddEventHandler("AdminMenu:giveMoneyCash", function(target, amount)
    ...
    -- Refresh player data in admin menu
    refreshPlayer(target, "forAllStaff", true)
...
AddEventHandler("Null:staff2:getiduinfo", function(idunique, type)
    ...
    if type == "idunique" then
        if SaveData.Players.Online[idunique] ~= nil then
            MySQL.Async.fetchAll('SELECT * FROM account_info WHERE license = @license',{
                ['@license'] = SaveData.Players.Online[idunique].identifier,
            }, function(result1)
                ...
                if result1[1] ~= nil then
                    TriggerClientEvent("AdminMenu:playerAsBeenSearchIdUnique", _src, SaveData.Players.Online[idunique].identifier, SaveData.Players.Online[idunique].jobs.name, SaveData.Players.Online[idunique].jobs2.name, SaveData.Players.Online[idunique].group, false, formattedLoadout, result1[1], {}, SaveData.Players.Online[idunique])
                else
                    TriggerClientEvent("AdminMenu:playerAsBeenSearchIdUnique", _src, SaveData.Players.Online[idunique].identifier, SaveData.Players.Online[idunique].jobs.name, SaveData.Players.Online[idunique].jobs2.name, SaveData.Players.Online[idunique].group, false, formattedLoadout, {}, {}, SaveData.Players.Online[idunique])
                end 
                local toSend = SaveData.Players.Online[idunique]
                toSend.loadout = formattedLoadout
                if result1[1] ~= nil then
                    toSend.account_info = result1[1]
                end
                local inJail = IN_JAIL[SaveData.Players.Online[idunique].identifier]
                if inJail then
                    toSend.injail = true
    ...
    elseif type == "discord" then
        ...
        if found == false then
            ...
            MySQL.Async.fetchAll('SELECT * FROM account_info WHERE license = @license',{
                ['@license'] = SaveData.Players.Online[found].identifier,
            }, function(result1)
                local formattedLoadout = {}
                for k,v in pairs(SaveData.Players.Online[found].loadout) do
                    local allcomp = {}
                    local getWeap1, getWeap2 = ESX.GetWeapon(v.name)
                    if getWeap2 ~= nil then 
                        allcomp = getWeap2.components 
                ...
                if result1[1] ~= nil then
                    TriggerClientEvent("AdminMenu:playerAsBeenSearchIdUnique", _src, SaveData.Players.Online[found].identifier, SaveData.Players.Online[found].jobs.name, SaveData.Players.Online[found].jobs2.name, SaveData.Players.Online[found].group, false, formattedLoadout, result1[1], {}, SaveData.Players.Online[found])
                else
                    TriggerClientEvent("AdminMenu:playerAsBeenSearchIdUnique", _src, SaveData.Players.Online[found].identifier, SaveData.Players.Online[found].jobs.name, SaveData.Players.Online[found].jobs2.name, SaveData.Players.Online[found].group, false, formattedLoadout, {}, {}, SaveData.Players.Online[found])
                end 
            end)
        end
    end
