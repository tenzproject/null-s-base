...
// Mapping des composants aux noms de dossiers
...
async function takeScreenshotForComponent(pedType, type, component, drawable, texture, cameraSettings) {
	...
	const folder = type === 'PROPS' ? propFolders[component] : componentFolders[component];
	if (!folder) return; // Skip si ce n'est pas un composant qu'on veut

	emitNet('takeScreenshot', `${folder}/${drawable}`, 'clothing');
	await Delay(500); 
	return;
}
...
RegisterCommand('screenshot', async (source, args) => {
	if (isScreenshotProcessRunning) {
		emit('chat:addMessage', {
			args: ['^1Error', 'Un processus de screenshot est déjà en cours']
		});
		return;
	}
	...
	basePos = GetEntityCoords(PlayerPedId(), false);
	...
	for (const modelHash of [GetHashKey('mp_m_freemode_01'), GetHashKey('mp_f_freemode_01')]) {
...
async function captureSpecificClothing(category, id, gender = 'male') {
    if (isScreenshotProcessRunning) {
        emit('chat:addMessage', {
            args: ['^1Error', 'Un processus de screenshot est déjà en cours']
        });
    ...
    const modelHashes = gender === 'male' ? [GetHashKey('mp_m_freemode_01')] :
                       gender === 'female' ? [GetHashKey('mp_f_freemode_01')] :
    ...
    for (const modelHash of modelHashes) {
        ...
        // Déterminer si c'est un prop ou un component
        const isProp = category in folderToProp;
        const componentId = isProp ? folderToProp[category] : folderToComponent[category];
        
        if (!componentId) {
            emit('chat:addMessage', {
                args: ['^1Error', `Catégorie invalide: ${category}`]
            });
            continue;
        }

        if (isProp) {
            await LoadPropVariation(ped, parseInt(componentId), parseInt(id));
            await takeScreenshotForComponent(pedType, 'PROPS', parseInt(componentId), parseInt(id));

			SendNUIMessage({
				type: category,
				value: parseInt(id),
				max: modelHashes.length,
			});
        } else {
            await LoadComponentVariation(ped, parseInt(componentId), parseInt(id));
            await takeScreenshotForComponent(pedType, 'CLOTHING', parseInt(componentId), parseInt(id));
...
async function captureCategoryClothing(category, gender = 'male') {
    if (isScreenshotProcessRunning) {
        emit('chat:addMessage', {
            args: ['^1Error', 'Un processus de screenshot est déjà en cours']
        });
    ...
    for (const modelHash of modelHashes) {
		...
		if (isProp) {
            const variationCount = GetNumberOfPedPropDrawableVariations(ped, parseInt(componentId));
            for (let i = 0; i < variationCount; i++) {
                await LoadPropVariation(ped, parseInt(componentId), i);
                await takeScreenshotForComponent(pedType, 'PROPS', parseInt(componentId), i);
                SendNUIMessage({
                    type: category,
                    value: i,
                    max: variationCount,
            ...
            const variationCount = GetNumberOfPedDrawableVariations(ped, parseInt(componentId));
            for (let i = 0; i < variationCount; i++) {
                await LoadComponentVariation(ped, parseInt(componentId), i);
                await takeScreenshotForComponent(pedType, 'CLOTHING', parseInt(componentId), i);
                SendNUIMessage({
                    type: category,
                    value: i,
                    max: variationCount,
    ...
    startWeatherResource();
    SendNUIMessage({ end: true });
	SetEntityCoords(ped, config.greenScreenPosition.x, config.greenScreenPosition.y, config.greenScreenPosition.z, false, false, false);
    DestroyAllCams(true);
    DestroyCam(cam, true);
    RenderScriptCams(false, false, 0, true, false, 0);
    camInfo = null;
...
RegisterCommand('screenshotitem', async (source, args) => {
    const [category, id, gender] = args;
    if (!category || !id) {
        emit('chat:addMessage', {
            args: ['^1Usage', '/screenshotitem [category] [id] [gender(optional)]']
        });
        return;
    }
    await captureSpecificClothing(category, id, gender);
});

RegisterCommand('screenshotcategory', async (source, args) => {
    const [category, gender] = args;
    if (!category) {
        emit('chat:addMessage', {
            args: ['^1Usage', '/screenshotcategory [category] [gender(optional)]']
        });
        return;
    }
    await captureCategoryClothing(category, gender);
});
...
setImmediate(() => {
	emit('chat:addSuggestions', [
		...
		{
			name: '/screenshotitem',
			help: 'generate a specific clothing item screenshot',
			params: [
				{name:"category", help:"The category of the clothing item (e.g. head_1, mask_1, etc.)"},
				{name:"id", help:"The ID of the clothing item"},
				{name:"male/female/both", help:"The gender to take a screenshot of (optional)"},
			]
		},
		{
			name: '/screenshotcategory',
			help: 'generate all clothing items in a category',
			params: [
				{name:"category", help:"The category of the clothing items (e.g. head_1, mask_1, etc.)"},
				{name:"male/female/both", help:"The gender to take a screenshot of (optional)"},
			]
		}
	])
...
on('onResourceStart', (resourceName) => {
    ...
    emit('chat:addSuggestion', '/screenshot', 'Capture tous les vêtements');
    emit('chat:addSuggestion', '/screenshotitem', 'Capture un vêtement spécifique', [
        {name: 'category', help: 'Catégorie (ex: pants_1, tshirt_1, etc.)'},
        {name: 'id', help: 'ID du vêtement'},
        {name: 'gender', help: 'Genre (male/female/both) - optionnel'}
    ]);
    emit('chat:addSuggestion', '/screenshotcategory', 'Capture toute une catégorie de vêtements', [
        {name: 'category', help: 'Catégorie (ex: pants_1, tshirt_1, etc.)'},
        {name: 'gender', help: 'Genre (male/female/both) - optionnel'}
    ]);
});
