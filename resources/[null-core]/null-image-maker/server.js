...
// Base paths
const Null_UI_IMAGES = 'resources/[null]/null-core/images';
const TEMP_PATH = 'resources/[null]/null-image-maker/temp';
...
function ensureDir(dirPath) {
    try {
        ...
        console.error(`[ImageMaker] Failed to create dir: ${dirPath}`, e.message);
    }
}
...
function getTargetPath(type, filename) {
    switch(type) {
        case 'vehicle':
            return `${Null_UI_IMAGES}/vehicles/${filename}.webp`;
        case 'weapon':
            return `${Null_UI_IMAGES}/weapons/${filename}.webp`;
        case 'ped':
            return `${Null_UI_IMAGES}/peds/${filename}.webp`;
        case 'prop':
            return `${Null_UI_IMAGES}/props/${filename}.webp`;
        case 'clothing':
            return `${Null_UI_IMAGES}/clothes/${filename}.webp`;
        case 'utils':
            return `${Null_UI_IMAGES}/clothes/utils/${filename}.png`;
        default:
...
async function processAndSave(tempPngPath, finalPath, greenRemoval) {
    try {
        ...
        console.error(`[ImageMaker] Process error: ${e.message}`);
...
onNet('null:imagemaker:capture', async (type, filename, greenRemoval) => {
    ...
    try {
        exports['screenshot-basic'].requestClientScreenshot(
            ...
            async (err, savedPath) => {
                if (err) {
                    console.error(`[ImageMaker] Screenshot error: ${err}`);
        ...
        console.error(`[ImageMaker] Capture error: ${e.message}`);
        emitNet('null:imagemaker:captureResult', src, false, filename, type);
    }
});
...
function scanClothingDir(gender) {
    const basePath = `${Null_UI_IMAGES}/clothes/${gender}`;
    const result = {};
    try {
...
function scanUtilsDir(gender) {
    const basePath = `${Null_UI_IMAGES}/clothes/utils/${gender}`;
    const result = {};
    try {
        if (!fs.existsSync(basePath)) return result;
        ...
        for (const type of types) {
            ...
            try {
                ...
                if (stat.isDirectory()) {
                    result[type] = scanDirectory(typePath, '.png');
                }
            } catch(e) {}
        }
    } catch(e) {}
...
onNet('null:imagemaker:scanImages', () => {
    ...
    const data = {
        vehicles: scanDirectory(`${Null_UI_IMAGES}/vehicles`, '.webp'),
        weapons: scanDirectory(`${Null_UI_IMAGES}/weapons`, '.webp'),
        peds: scanDirectory(`${Null_UI_IMAGES}/peds`, '.webp'),
        props: scanDirectory(`${Null_UI_IMAGES}/props`, '.webp'),
        clothing: {
...
console.log('[ImageMaker] Server v2.0 loaded');
