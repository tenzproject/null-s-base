...
local function PointingStopped()
    ...
    if Config.Animations.PersistentEmoteAfterPointing and IsInAnimation then
...
-- Commands & KeyMapping --
if Config.Animations.PointingEnabled then
    RegisterCommand('pointing', function()
        if IsPedInAnyVehicle(PlayerPedId(), false) and not Config.Animations.PointingKeybindInCarEnabled then
            return
        end
        StartPointing()
    end, false)

    if Config.Animations.PointingKeybindEnabled then
        RegisterKeyMapping("pointing", "Finger pointing", "keyboard", Config.Animations.PointingKeybind)
    end
