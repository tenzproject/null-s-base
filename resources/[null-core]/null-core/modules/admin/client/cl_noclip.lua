-- FREECAM 
RegisterCommand("adminFreecam", function()
    if ESX.HasPermissions("freecam") and nTable.staffMode then
        ToogleNoClip(true)
    end
end, false)
RegisterKeyMapping('adminFreecam', 'Raccourci Freecam', 'keyboard', 'F10')

-- --NOCLIP
-- RegisterCommand("adminNoclip", function()
--     if ESX.HasPermissions("noclip") and nTable.staffMode then
...
function ToogleNoClip(isFreecam)
    if nTable.noclip then
        ...
        if nTable.isSpectateMod then 
            ESX.ShowNotification('❌ Vous ne pouvez pas activer le Noclip en mode Spectate') 
            nTable.noclip = false
            return
        end
        nTable.noclip = true
...
local controls = {12, 13, 14, 15, 16, 17, 18, 19, 50, 85, 96, 97, 99, 115, 180, 181, 198, 261, 262, 30,31,32,33}
...
function CameraLoop(freecam)
    if not IsFreecamEnabled() or PlayerState.inPauseMenu then
        return
    end
