...
local ShowRemaining = LPH_NO_VIRTUALIZE(function()
    null.fct.draw.Text2(null.fct.format.SecondsToClock(RemainingTime), 0.4, 0.485, 0.170, {49, 127, 99})
end)

local Timer = LPH_NO_VIRTUALIZE(function()
...
local ButtonMessage = LPH_NO_VIRTUALIZE(function(text)
    BeginTextCommandScaleformString("STRING")
    AddTextComponentScaleform(text)
    EndTextCommandScaleformString()
end)

local Button = LPH_NO_VIRTUALIZE(function(ControlButton)
    PushScaleformMovieMethodParameterButtonName(ControlButton)
end)

local SetupScaleform = LPH_NO_VIRTUALIZE(function(scaleform, message, button, message2, buttons, message3, button2)
    ...
    ButtonMessage("Change horizontal")
    PopScaleformMovieFunctionVoid()
...
local func_reset = LPH_NO_VIRTUALIZE(function()
    selected = 1
    selectedNode = {
        [1] = {0, math.random(1, 4)},
        [2] = {0, math.random(1, 4)},
...
local func_1920 = LPH_NO_VIRTUALIZE(function(param0)
    return param0 / 1920.0
end)

local func_1080 = LPH_NO_VIRTUALIZE(function(param0)
    return param0 / 1080.0
end)

local func_drawtext = LPH_NO_VIRTUALIZE(function(param0, param1, param2, param3, param4, param5, param6)
...
local CreateScale = LPH_NO_VIRTUALIZE(function(sType)
	scaleType = SetupScaleform("instructional_buttons", "Confirm", 191, "Change", {174, 175}, "Exit", 194 )
end)

null.fct.draw.StartHotwire = LPH_NO_VIRTUALIZE(function(cb)
    minigameLoop = true
    time = 3000
    ...
    while minigameLoop do
        for k, v in pairs(drawTable) do
            null.fct.draw.DrawRetroSprite(v[1], v[2], v[3], v[4])
        end
        if not win then
