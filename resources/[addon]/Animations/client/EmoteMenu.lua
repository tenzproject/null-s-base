...
if Config.Animations.MenuPosition then
    if Config.Animations.MenuPosition == "left" then
        menuPosition = leftPosition
    elseif Config.Animations.MenuPosition == "right" then
        menuPosition = rightPosition
    end
end

if Config.Animations.CustomMenuEnabled then
    local RuntimeTXD = CreateRuntimeTxd('Custom_Menu_Head')
    CreateRuntimeTextureFromImage(RuntimeTXD, 'Custom_Menu_Head', 'interaction_bgd.png')
...
mainMenu = NativeUI.CreateMenu(Config.Animations.MenuTitle or "", "", menuPosition["x"], menuPosition["y"], Menuthing, Menuthing)
...
if Config.Animations.FavKeybindEnabled then
    RegisterCommand('emotefav', function() FavKeybind() end)
    RegisterKeyMapping("emotefav", "Execute your favorite emote", "keyboard", Config.Animations.FavKeybind)
...
function AddEmoteMenu(menu)
    ...
    local favEmotes = {}
    if not Config.Animations.SqlKeybinding then
   ...
   propmenu.OnListSelect = function(menu, item, itemIndex, listIndex)
    ...
    submenu.OnItemSelect = function(sender, item, index)
        if Config.Animations.Search and EmoteTable[index] == Translate('searchemotes') then
...
if Config.Animations.Search then
    local ignoredCategories = {
        ["Walks"] = true,
        ["Expressions"] = true,
        ["Shared"] = not Config.Animations.SharedEmotesEnabled
    }

    function EmoteMenuSearch(lastMenu)
        local favEnabled = not Config.Animations.SqlKeybinding and Config.Animations.FavKeybindEnabled
        ...
        if input ~= nil then
            ...
            if #results > 0 then
                ...
                for k, v in pairs(results) do
                    ...
                    if v.table == "Dances" and Config.Animations.SharedEmotesEnabled then
...
function AddInfoMenu(menu)
    ...
    for _,v in ipairs(Config.Animations.Credits) do
...
if Config.Animations.PreviewPed then
    ShowPedPreview(mainMenu)
end
if Config.Animations.WalkingStylesEnabled then
    AddWalkMenu(mainMenu)
end
if Config.Animations.ExpressionsEnabled then
